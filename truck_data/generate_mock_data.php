...
// I made a modification to the code to alternate the recycling collections so
// odd rounds are collected on odd weeks and even rounds are collected on even weeks.
// Also added an index to the SQL table on 'Property_Number' to speed up returns as the property number is the joiner.
//
// Going to work out a SELECT statement to get last 16 weeks of (simulated) general and recycling loads
//
// Daniel
...
foreach($rounds as $round) {
   ...
   if($round%2==0) {
       print_r("Recycling is on even weeks \n\r");
   } else {
       print_r("Recycling is on odd weeks \n\r");
   }
   /********************************************************************************/
   ...
   for($i=32;$i<48;$i++) {
       ...
       $rundate->setTime(06,rand(-5,10),rand(0,30));   // reset and randomise time for run start
       ...
       $rundate->setTime(07,rand(-5,10),rand(0,30));   // reset and randomise time for run start
       $truckid = rand(1,27);                          // no idea which truck; just randomise;
       $bintype = 4;                                   // static for now; would come off QR code
       $binsize = 240;                                 // determined by bin type

       if(($i%2==0)xor($round%2==0)) {                 // Recycling every fortnight only, alternating week by round
           foreach($properties as $property) {
               //echo ".";
               $random = '+'.$shift[$j].' seconds';
               $rundate->modify($random);
